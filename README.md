...
- [概要](#概要)
- [コンセプト概要](#コンセプト概要)
  - [プロジェクトの魅力：分散型 AI の新時代](#プロジェクトの魅力分散型-ai-の新時代)
  - [アプリの特徴と ICP 活用メリット](#アプリの特徴と-icp-活用メリット)
  - [現在のモデル一覧](#現在のモデル一覧)
  - [プロジェクト構造](#プロジェクト構造)
  - [将来の展望](#将来の展望)
- [開発リソース](#開発リソース)
...
Trust Chat Platform は、Internet Computer Protocol (ICP) 上に構築された分散型 AI チャットプラットフォームである。ユーザーはオンチェーンで記録された透明性の高い AI モデル情報を確認しながら、複数の AI モデルから選択してチャットを楽しめる。特に、学習時に恣意的な情報が与えられる可能性があるモデルに対して、ユーザーが安心して利用できる選択肢を提供する。
...
Trust Chat Platform は、従来の中央集権型 AI プラットフォームを分散的に管理することで、以下のようなメリットの実現を目指している：
...
- **将来展望**: [オンチェーン AI 推論](https://internetcomputer.org/ecosystem)、分散ガバナンス（DAO による管理）、変更履歴の可視化ツールの開発を計画

## アプリの特徴と ICP 活用メリット

本プラットフォームが ICP を活用して実現する「差別化された機能」と、それによってユーザーが得られるメリットをカテゴリ別にまとめている。
...
1. **英会話講師** (OpenAI GPT-4o-mini)
   - 日本人向けの英会話講師。語彙や文法の修正を丁寧に行う。
   - キャラクター名: Lady Victoria

2. **ドラえもん** (OpenAI o3-mini)
   - 22世紀から来た猫型ロボット。ひみつ道具を出して、のび太くんの手助けをする。
   - キャラクター名: ドラえもん

## プロジェクト構造

Trust Chat Platform は以下のキャニスター（スマートコントラクト）で構成されている：
...
各 AI モデルを独立したキャニスターとして実装することで、以下のメリットを想定している：
...
現状では、キャニスターのコード変更履歴は IC ネットワーク上に記録されているが、一般ユーザーが簡単に閲覧するツールは限られている。今後は以下の機能を開発予定である：

1. モックモデルで実装しているチャット機能を実装
2. II認証の実装
3. 事業者、モデル開発者によるAIモデルデプロイ機能実装（キャニスターを動的にデプロイ）
4. モデル開発者によるAIモデルの変更履歴ダッシュボード実装
3. **アラート機能**: 重要な変更があった場合にユーザーに通知する機能
4. **差分表示**: 変更前後のコードやシステムプロンプトの差分を表示する機能
...
現状では AI 推論処理は外部 API に依存しているが、将来的には以下の方向性で完全なオンチェーン AI を目指す：
